...
const Utils = {
  ...
  async adbDevHelperHttpService(packageName, esCmd) {
    ...
    await ADB.startService(
      packageName,
      activityName,
      `com.mz.fastapp.DevHelperHttpService`,
      `--es cmd "${cmdBase64}"`
    );
  },
  localhost: "http://127.0.0.1",
  async syncToRemotOneFile(packageName, fileName) {
    let ppoboxExe = fpath.join(
      Utils.context.asAbsolutePath("tools"),
      "ppobox.exe"
    );
    let workspace = await Utils.getWorkspaceFolder();
  ...
  async syncToRemotDiffFiles(packageName) {
    ...
    let tmpFileSummaryJson = fpath.join(
      process.env.TEMP,
      "md5-workspace-summary.json"
    );
    ...
    try {
      ...
      for (let index = 0; index < syncFiles.length; index++) {
        try {
          ...
          // skip dir
          if (
            fs.lstatSync(fpath.join(workspace, syncFiles[index])).isDirectory()
          ) {
  ...
  getWebViewContent(context, templatePath) {
    ...
    html = html.replace(
      ...
      (m, $1, $2) => {
        return (
          $1 +
          vscode.Uri.file(fpath.resolve(dirPath, $2))
