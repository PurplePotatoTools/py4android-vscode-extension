...
const ADB = {
    ...
    async queryApkInfo(packageName) {
        try {
            return execSync(`${getAdbPath()} shell pm list packages -i ${packageName}`).toString().trim();
        } catch (error) {
            return "";
        }
    },
    ...
    async BuildInstallRun(apkPath, packageName, className) {
        ...
        return [cmd, `${getAdbPath()} shell am start ` + `-e startupType reinit` + ` -n ${packageName}/${className}`];
    },
};
